...
function XRDetailInner({ xrd, name, namespace }: XRDetailInnerProps) {
  ...
  const resourceRefs: { apiVersion: string; kind: string; name: string; namespace?: string }[] =
    item?.jsonData?.status?.resourceRefs ?? [];
            ...
            <SectionBox title="Composed Resources">
              <Table
                ...
                columns={[
                  ...
                  { header: 'Namespace', accessorFn: (r: any) => r.namespace ?? '-' },
                ]}
              />
            </SectionBox>
          )}
